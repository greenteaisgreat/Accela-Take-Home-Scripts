...
2. Accela EMSE Overview & API Reference
...
2. A small helper, e.g.:
function dateDiffInDays(d1, d2) { /*…*/ }.

3. A short code snippet at the bottom showing how you’d call it with an example array of
CapIDs (e.g., from `aa.cap.getCapID("2025PA-000001", "00000", "00001").getOutput()`).

4. After every `aa.cap.getCap(...)`, `aa.cap.updateCapStatus(...)`, and
`aa.cap.createCapComment(...)` call, include a `getSuccess()` check.

5. Use `logDebug(...)` (or comments) to indicate success/failure or skipping logic.
API Hints:
